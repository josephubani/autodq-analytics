...
The current language version is **ADQL 2.4**. A conforming AutoDQ installation
exposes it as:

```python
from autodq import ADQL_LANGUAGE_VERSION

assert ADQL_LANGUAGE_VERSION == "2.4"
```
...
```text
ADQL 2.4 parser
ADQL 2.4 AutoDQ runtime
ADQL 2.4 notebook host
```
