...
The current language version is **ADQL 2.5**. A conforming AutoDQ installation
exposes it as:

```python
from autodq import ADQL_LANGUAGE_VERSION

assert ADQL_LANGUAGE_VERSION == "2.5"
```
...
```text
ADQL 2.5 parser
ADQL 2.5 AutoDQ runtime
ADQL 2.5 notebook host
```
