...
The current language version is **ADQL 2.1**. A conforming AutoDQ installation
exposes it as:

```python
from autodq import ADQL_LANGUAGE_VERSION

assert ADQL_LANGUAGE_VERSION == "2.1"
```
...
```text
ADQL 2.1 parser
ADQL 2.1 AutoDQ runtime
ADQL 2.1 notebook host
```
