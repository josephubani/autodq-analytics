...
The current language version is **ADQL 2.0**. A conforming AutoDQ installation
exposes it as:

```python
from autodq import ADQL_LANGUAGE_VERSION

assert ADQL_LANGUAGE_VERSION == "2.0"
```
...
```text
ADQL 2.0 parser
ADQL 2.0 AutoDQ runtime
ADQL 2.0 notebook host
```
