...
"""
    ...
    def _build_automation_section(self, automation):
        if automation is None:
            return ""

        stage_rows = []

        for stage in automation.stages:
            stage_rows.append(
                f"""
                <tr>
                    <td>{escape(stage.name)}</td>
                    <td>{escape(stage.status)}</td>
                    <td>{escape(stage.message)}</td>
                    <td>{stage.duration_seconds:.4f}s</td>
                </tr>
                """
            )
        ...
        next_actions = "".join(
            f"<li>{escape(item)}</li>"
            for item in automation.next_actions
        ) or "<li>No follow-up action is required.</li>"
        return f"""
        <div class="section card">
            <h2 class="section-title">Automatic Workflow</h2>
            <div class="grid">
                <div class="card"><h3>Mode</h3>
                    <div class="metric">{escape(automation.config.mode)}</div></div>
                <div class="card"><h3>Completed</h3>
                    <div class="metric">{automation.completed_count}</div></div>
                <div class="card"><h3>Reused</h3>
                    <div class="metric">{automation.reused_count}</div></div>
                <div class="card"><h3>Skipped</h3>
                    <div class="metric">{automation.skipped_count}</div></div>
                <div class="card"><h3>Failed</h3>
                    <div class="metric">{automation.failed_count}</div></div>
            </div>
            <table>
                <tr><th>Stage</th><th>Status</th><th>Message</th>
                    <th>Duration</th></tr>
                {''.join(stage_rows)}
            </table>
            <h3>Next Actions</h3><ul>{next_actions}</ul>
        </div>
        """

    def _build_dashboard_section(self, dashboard):
        ...
        metric_cards = "".join(
            ...
            <div class="card">
                ...
                <div class="metric">{escape(metric.display)}</div>
                <div class="metric-small">
        ...
        return f"""
        <div class="section card">
            <h2 class="section-title">Generated Dashboard</h2>
            <p>
                {escape(dashboard.title)} · {escape(dashboard.stage.title())}
                stage · {dashboard.chart_count} visualization(s) ·
                {dashboard.preview_row_count} preview row(s)
            </p>
            <div class="grid">{metric_cards}</div>
        </div>
        """

    def _build_adql_section(self, history):
        if not history:
            return ""
        ...
        for run in reversed(history[-10:]):
            ...
            status_class = "success" if run.success else "high"
            rows.append(
                f"""
                <tr>
                    <td>{escape(run.source_name)}</td>
                    <td><span class="badge {status_class}">{status}</span></td>
                    <td>{run.statement_count}</td>
                    <td>{run.completed_count}</td>
                    <td>{run.failed_count}</td>
                    <td>{run.duration_seconds:.4f}s</td>
                </tr>
                """
            )
        ...
        return f"""
        <div class="section card">
            <h2 class="section-title">ADQL Query History</h2>
            <table>
